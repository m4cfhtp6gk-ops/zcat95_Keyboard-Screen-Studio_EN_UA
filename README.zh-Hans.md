...
- **Windows：** v1.4.0，已迁移到 Avalonia UI，作为当前正式发布平台。
- **macOS：** 已保留跨平台项目和发布脚本，Intel 与 Apple Silicon 包仍待真实设备测试，暂不作为稳定版本发布。
- **Linux：** 暂未发布；核心数据、渲染与平台接口已为后续适配留出空间。

## 主要功能

- 27 个内置主题，涵盖监控、时间、资讯、音乐、点阵与图片时间——包括分页轮播的硬件监控、汇率、带走势图的币安加密货币、番茄钟、GitHub 贡献网格，以及基于 alerts.in.ua 数据的乌克兰防空警报（仅供参考，切勿作为唯一警报来源）。另有主题轮播、每主题独立刷新间隔、12/24 小时制与 °C/°F 单位，以及去除机密信息的设置导出/导入。
...
下载 Release 中的 `KeyboardScreenStudio-v1.4.0-win-x64.zip`，解压后运行 `KeyboardScreenStudio.exe`。发布包为自包含版本，无需另装 .NET。
