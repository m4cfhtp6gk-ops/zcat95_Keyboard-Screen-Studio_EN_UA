...
SCREEN = [
    ...
    ("ClaudeOAuthNoToken", "The sign-in returned no token", "Вхід не повернув токен", "登录未返回令牌"),
    ("ClaudeOAuthExchangeFailed",
     "Anthropic rejected the sign-in (HTTP {0})",
     "Anthropic відхилив вхід (HTTP {0})",
     "Anthropic 拒绝了登录（HTTP {0}）"),
    ("ClaudeOAuthStateMismatch",
     "That code is from a different sign-in - start again",
     "Цей код з іншого входу - почніть заново",
     "该代码来自另一次登录 - 请重新开始"),
    ("ClaudeOAuthNoCode", "Paste the code the browser showed", "Вставте код, який показав браузер", "请粘贴浏览器显示的代码"),
    ("ClaudeOAuthSource", "your Claude account (signed in here)", "ваш акаунт Claude (вхід тут)", "你的 Claude 账户（在此登录）"),
    ("ClaudeOAuthExpired",
     "Your Claude sign-in expired - sign in again",
     "Ваш вхід Claude протух - увійдіть знову",
     "你的 Claude 登录已过期 - 请重新登录"),
    ("ClaudeTokenRejected",
