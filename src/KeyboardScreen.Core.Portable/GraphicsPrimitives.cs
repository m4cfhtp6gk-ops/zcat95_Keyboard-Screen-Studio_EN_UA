...
public sealed class StreamGeometryContext:IDisposable{private readonly SKPath _path;internal StreamGeometryContext(SKPath path)=>_path=path;public void BeginFigure(Point p,bool f,bool c)=>_path.MoveTo((float)p.X,(float)p.Y);public void BezierTo(Point a,Point b,Point c,bool s,bool j)=>_path.CubicTo((float)a.X,(float)a.Y,(float)b.X,(float)b.Y,(float)c.X,(float)c.Y);public void LineTo(Point p,bool s,bool j)=>_path.LineTo((float)p.X,(float)p.Y);public void Dispose(){}}