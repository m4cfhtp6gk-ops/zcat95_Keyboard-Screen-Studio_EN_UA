...
- Exchange rates come from a selectable source: the fawazahmed0 currency-api dataset served from the jsDelivr CDN (CC0, updated daily; the default, with a fallback to ExchangeRate-API), the ExchangeRate-API open endpoint (https://www.exchangerate-api.com, requires attribution: rates by ExchangeRate-API), or the National Bank of Ukraine's public exchange-rate API (https://bank.gov.ua) with other pairs crossed through UAH. Daily data; the screen shows the date the rates are for.
